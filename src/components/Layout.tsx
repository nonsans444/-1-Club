...
export function Layout({ children, user, loading }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#F8F9FA] flex flex-col">
      <nav className="h-20 border-b border-gold-accent/20 px-4 sm:px-10 flex items-center justify-between relative z-10 bg-bg-deep/80 backdrop-blur-md">
...
export function DiscordLoginLink({ fullWidth = false }: { fullWidth?: boolean }) {
  ...
  return (
    <button
      onClick={handleLogin}
      className={cn(
        "bg-gold-accent hover:bg-gold-hover text-black px-10 py-4 font-bold uppercase tracking-[0.3em] text-xs transition-colors flex items-center justify-center gap-3",
        fullWidth && "w-full"
      )}
    >
